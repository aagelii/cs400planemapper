...
public class CS400Graph<T> implements GraphADT<T> {
  ...
  protected class Path implements Comparable<Path> {
    ...
    public Path(Path copyPath, Edge extendBy) {
      this.start = copyPath.start; // copy start
      this.distance = copyPath.distance+extendBy.weight; // copy the distance and add weight of edge
      this.dataSequence = new LinkedList<>(); // make new LinkedList
      this.dataSequence.addAll(copyPath.dataSequence); // copy original dataSequence
      this.dataSequence.add(extendBy.target.data); // add the extended edges target
      this.end = extendBy.target; // make end the target of extended edge
    }
  ...
  protected Path dijkstrasShortestPath(T start, T end) {
    if(start == end){ // if same nodes, return itself in a path
      return new Path(new Vertex(start));
    }
    if(start==null || end==null) { // if the given data is null, throw exception
      throw new NoSuchElementException("NO PATH FOUND");
    }
    Path shortest= new Path(new Vertex(start)); // path to hold shortest, starting with start node
    PriorityQueue<Path> frontier = new PriorityQueue<>(); // PriorityQueue to hold frontier
    PriorityQueue<Path> paths = new PriorityQueue<>(); // PriorityQueue of paths
    Vertex begin = vertices.get(start); // vertex that represents start data
    ArrayList<Vertex> visited= new ArrayList<>(); // ArrayList of visited nodes
    visited.add(new Vertex(start)); // add start node to visited
    Vertex last = vertices.get(end); // vertex that represents end data

    if(begin==null || last==null) { // if vertex's aren't found, throw exception
      throw new NoSuchElementException("NO PATH FOUND");
    }

    for(int i =0; i<begin.edgesLeaving.size();i++){ // add all the edges leaving start node to frontier
      frontier.add(new Path(shortest, begin.edgesLeaving.get(i)));
    }

    while(!(frontier.isEmpty())){ // while frontier is not empty
      shortest=frontier.peek(); // set shortest path to beginning of frontier
      if(frontier.peek().start.data==start && frontier.peek().end.data==end) {
        // if the start data and end data match parameters for a path in the frontier, add to paths
        paths.add(frontier.peek());
      }
      visited.add(frontier.poll().end); // dequeue front of frontier and add end of path to visited
      for(int i =0; i<shortest.end.edgesLeaving.size();i++){ // add edgesLeaving the end of current
                                                             // to shortest path
        if(!visited.contains(shortest.end.edgesLeaving.get(i).target)) { // if already visited the
                                                                         // node, skip it
          frontier.add(new Path(shortest, shortest.end.edgesLeaving.get(i)));
        }
      }
    }
    if(paths.size()==0){ // if paths is empty, it means no paths were found, so throw exception
      throw new NoSuchElementException("NO PATH FOUND");
    }
    shortest= paths.poll(); // dequeue front of paths to get shortest path

    if(shortest.start.data!=start &&shortest.end.data!=end){ // checks if start and end are correct
                                                             // data, otherwise throw exception
      throw new NoSuchElementException("NO PATH FOUND");
    }
